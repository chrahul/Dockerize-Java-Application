= Step# 2: Packaging Application using Docker
...
. Show image history:
	...
	docker image history rahulch/greeting
...
. Build the Docker image:
	...
	mvn -Dmaven.repo.local=./repository clean package
	tar cf repository.tar.gz ./repository
	sudo docker image build -t rahulch/greeting .
...
. Build Docker image using this custom JRE:
	...
	sudo docker image build -f Dockerfile.jre -t rahulch/greeting:jre-slim .

. List Docker images and show the difference in sizes:

	ubuntu@ip-172-31-35-132:~$ sudo docker image ls
	REPOSITORY           TAG                 IMAGE ID            CREATED             SIZE
	rahulch/greeting   jre-slim            4334d1ebec46        2 minutes ago       160MB
	rahulch/greeting   latest              079dd8de2731        5 minutes ago       489MB

. Run the container:

	sudo docker container run -d -p 8080:8080 rahulch/greeting:jre-slim
